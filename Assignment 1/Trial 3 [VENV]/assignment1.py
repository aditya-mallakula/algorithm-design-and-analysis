...
def main():
    ...
    if not input_filename.rstrip('.txt').isalpha() and input_filename.rstrip('.txt').isalnum():
        output_filename = f'Output{''.join(filter(str.isdigit, input_filename))}.txt'
        verification_filename = f'OutputToBeVerified{''.join(filter(str.isdigit, input_filename))}.txt'
